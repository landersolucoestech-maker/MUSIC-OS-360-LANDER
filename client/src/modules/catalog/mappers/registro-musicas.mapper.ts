...
export function exportCriadaPorIA(r: Record<string, unknown>): string {
  return r["criada_por_ia"] === true || r["criadaPorIA"] === "sim" ? "Sim" : "Não";
}

export function exportTipoIA(r: Record<string, unknown>): string {
  const v = r["tipo_ia"] ?? r["tipoIA"];
  return typeof v === "string" ? v : "";
}

export function exportElementosIA(r: Record<string, unknown>): string {
  const hasContent = (field: unknown): boolean => {
    if (!field || typeof field !== "object") return false;
    const obj = field as Record<string, unknown>;
    return !!(obj["ferramenta"] || obj["prompt"]);
  };
  const elems: string[] = [];
  if (hasContent(r["ia_harmonia"] ?? r["iaHarmonia"])) elems.push("Harmonia");
  if (hasContent(r["ia_melodia"] ?? r["iaMelodia"])) elems.push("Melodia");
  if (hasContent(r["ia_letra"] ?? r["iaLetra"])) elems.push("Letra");
  return elems.join(", ");
}

export function exportParticipantes(r: Record<string, unknown>): string {
  const parts = r["participantes"] as Array<{ nome?: string; classeFuncao?: string; percentual?: string; link?: string }> | null | undefined;
  if (!Array.isArray(parts) || parts.length === 0) return "";
  return parts
    .map((p) => {
      const segs = [p.nome ?? ""].concat(
        [p.classeFuncao ?? "", p.percentual ? `${p.percentual}%` : "", p.link ?? ""].filter(Boolean),
      );
      return segs.join(" | ");
    })
    .filter((s) => s.trim() !== "")
...
export function exportOutrosTitulos(r: Record<string, unknown>): string {
  return normalizeStringArray(r["outros_titulos"] ?? r["outrosTitulos"]).join(", ");
}

export function exportReferenciasConexas(r: Record<string, unknown>): string {
  return normalizeStringArray(r["referencias_conexas"] ?? r["referenciasConexas"]).join(", ");
}

export function exportLetraCompleta(r: Record<string, unknown>): string {
  const v = r["letra_completa"] ?? r["letraCompleta"];
  return typeof v === "string" ? v : "";
}
...
export function importParseParticipantes(val: string): ParticipanteForm[] {
  return (val || "")
    .split(";")
    .map((s) => s.trim())
    .filter(Boolean)
    .map((seg) => {
      ...
      return {
        id: crypto.randomUUID(),
        nome: parts[0] ?? "",
        classeFuncao: parts[1] ?? "",
        percentual: (parts[2] ?? "").replace("%", "").trim(),
        link: parts[3] ?? "",
      };
    });
}

export function importSplitArray(val: string): string[] {
  return (val || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
export function importParseElementosIA(
  val: string,
  tipoIA: string,
): { ia_harmonia: ObraIAElement | null; ia_melodia: ObraIAElement | null; ia_letra: ObraIAElement | null } {
  const elems = (val || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
  const make = (present: boolean): ObraIAElement | null =>
    present ? { ferramenta: tipoIA || "IA", prompt: "" } : null;
  return {
    ia_harmonia: make(elems.includes("harmonia")),
    ia_melodia: make(elems.includes("melodia")),
    ia_letra: make(elems.includes("letra")),
  };
}
...
export function formToObraPayload(input: FormToObraInput): Record<string, unknown> {
  ...
  return {
    ...
    criadaPorIA: input.criadaPorIA || null,
    tipo_ia: input.tipoIA || null,
    tipoIA: input.tipoIA || null,
    ia_harmonia: iaH,
    iaHarmonia: iaH,
    ia_melodia: iaM,
    iaMelodia: iaM,
    ...
    letraCompleta: input.letraCompleta || null,
