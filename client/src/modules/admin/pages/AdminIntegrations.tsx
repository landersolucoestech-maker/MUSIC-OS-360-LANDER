...
import {
  ...
  CheckCircle2, AlertCircle, Clock, XCircle, Globe, Building2,
  Languages, Timer, Lock, Eye, EyeOff, RefreshCw, Plus, Trash2,
  Copy, ToggleLeft, ToggleRight, Upload,
} from "lucide-react";

/* ── types ── */
type TabKey = "geral" | "email" | "seguranca" | "notificacoes" | "aparencia" | "webhooks" | "chaves-api" | "integracoes";
...
function Select({ options, defaultValue }: { options: string[]; defaultValue?: string }) {
  return (
    <select
      defaultValue={defaultValue}
      className="w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-[13px] text-white focus:outline-none focus:border-primary/40 appearance-none"
    >
    ...
    <div className="space-y-4">
      <Section title="Informações da Plataforma">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Nome da Plataforma"><Input defaultValue="MUSIC OS 360" /></Field>
          <Field label="URL da Plataforma"><Input defaultValue="musicos360.com" /></Field>
          <Field label="Fuso Horário">
            <Select options={["America/Sao_Paulo", "America/Manaus", "America/Fortaleza", "UTC"]} defaultValue="America/Sao_Paulo" />
          </Field>
          <Field label="Idioma Padrão">
            <Select options={["Português (Brasil)", "English", "Español"]} defaultValue="Português (Brasil)" />
          </Field>
          ...
          <Field label="Moeda Padrão">
            <Select options={["BRL — Real Brasileiro", "USD — Dólar", "EUR — Euro"]} defaultValue="BRL — Real Brasileiro" />
          </Field>
        </div>
      </Section>
      <Section title="Suporte & Contato">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Email de Suporte"><Input defaultValue="suporte@musicos360.com" type="email" /></Field>
          <Field label="Telefone de Suporte"><Input defaultValue="+55 11 99999-0000" /></Field>
          <Field label="Endereço" hint="Usado em documentos fiscais e contratos">
            <Input defaultValue="Rua das Artes, 123 — Sala 45, São Paulo, SP" />
          </Field>
  ...
  return (
    <div className="space-y-4">
      <Section title="Configuração SMTP">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Host SMTP"><Input placeholder="smtp.exemplo.com" /></Field>
          <Field label="Porta">
            <Select options={["465 (SSL)", "587 (TLS)", "25 (padrão)"]} defaultValue="587 (TLS)" />
          </Field>
          <Field label="Usuário SMTP"><Input placeholder="usuario@dominio.com" type="email" /></Field>
          ...
          <Field label="Nome do Remetente"><Input defaultValue="MUSIC OS 360" /></Field>
          <Field label="Email do Remetente"><Input defaultValue="noreply@musicos360.com" type="email" /></Field>
        </div>
      </Section>
      ...
      <Section title="Política de Senhas">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Comprimento Mínimo">
            <Select options={["8 caracteres", "10 caracteres", "12 caracteres", "16 caracteres"]} defaultValue="12 caracteres" />
          </Field>
          <Field label="Expiração de Senha">
            <Select options={["Nunca", "30 dias", "60 dias", "90 dias"]} defaultValue="90 dias" />
          </Field>
        </div>
        <div className="space-y-0">
      ...
      <Section title="Sessão">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Timeout de Sessão" hint="Usuário é desconectado após inatividade">
            <Select options={["30 minutos", "1 hora", "4 horas", "8 horas", "24 horas"]} defaultValue="4 horas" />
          </Field>
          <Field label="Máximo de Sessões Simultâneas">
            <Select options={["1 sessão", "2 sessões", "5 sessões", "Ilimitado"]} defaultValue="5 sessões" />
          </Field>
        </div>
      </Section>
...
function TabNotificacoes() {
  return (
    <div className="space-y-4">
      ...
      <Section title="Frequência de Resumos">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Resumo de Atividade">
            <Select options={["Diário", "Semanal", "Mensal", "Desativado"]} defaultValue="Semanal" />
          </Field>
          <Field label="Horário de Envio">
            <Select options={["08:00", "09:00", "10:00", "18:00"]} defaultValue="09:00" />
          </Field>
        </div>
      </Section>
      <SaveBar />
  ...
  return (
    <div className="space-y-4">
      ...
      <Section title="Identidade Visual">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Nome da Plataforma (sidebar)"><Input defaultValue="MUSIC OS 360" /></Field>
          <Field label="Slogan (sidebar)"><Input defaultValue="ERP OPERACIONAL MUSICAL" /></Field>
...
const MOCK_KEYS = [
  { id: "key-1", name: "Produção Principal", prefix: "sk_live_", key: "sk_live_••••••••••••••••3f9a", created: "2026-01-10T00:00:00Z", last_used: "2026-05-08T07:45:00Z", scopes: ["read", "write"] },
  { id: "key-2", name: "Integração Relatórios", prefix: "sk_live_", key: "sk_live_••••••••••••••••8b2c", created: "2026-03-01T00:00:00Z", last_used: "2026-05-07T12:00:00Z", scopes: ["read"] },
  { id: "key-3", name: "Webhook Interno", prefix: "sk_live_", key: "sk_live_••••••••••••••••1d7e", created: "2026-04-15T00:00:00Z", last_used: "2026-05-06T09:30:00Z", scopes: ["read", "write", "admin"] },
];
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-[12px] text-white/35">
          {MOCK_KEYS.length} chave{MOCK_KEYS.length !== 1 ? "s" : ""} ativa{MOCK_KEYS.length !== 1 ? "s" : ""}
        </p>
          ...
          {MOCK_KEYS.map(k => (
            <div key={k.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3" data-testid={`apikey-${k.id}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button className="text-white/20 hover:text-white/50 transition-colors" title="Copiar chave"><Copy className="h-3.5 w-3.5" /></button>
                  <button className="text-white/20 hover:text-red-400 transition-colors" title="Revogar"><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[12px] font-mono text-white/40 bg-white/[0.04] px-3 py-1.5 rounded-lg flex-1 truncate">{k.key}</span>
              </div>
      ...
      <Section title="Documentação">
        <p className="text-[12.5px] text-white/40 leading-relaxed">
          Use as chaves API para autenticar requisições à API REST do MUSIC OS 360.
          Inclua o header <span className="font-mono text-primary/70">Authorization: Bearer sk_live_...</span> em todas as requisições.
        </p>
        <button className="mt-2 text-[12px] text-primary hover:underline">Ver documentação completa →</button>
      </Section>
    </div>
  );
}
