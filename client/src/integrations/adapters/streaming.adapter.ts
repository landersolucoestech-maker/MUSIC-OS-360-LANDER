 ...
 *
 * Uso:
 *   import { getStreamingAdapter, getAdsAdapter } from "@/integrations/adapters/streaming.adapter";
...
import { makeMockStreamingProvider, makeMockAdsProvider } from "@/integrations/providers";
...
export function getStreamingAdapter(platform: StreamingPlatformId): IStreamingProvider {
  ...
  if (MOCK_MODE) {
    ...
    // import { BackendStreamingProvider } from "@/integrations/providers/backend/backend-streaming.provider";
...
export function getAdsAdapter(platform: AdsPlatformId): IAdsProvider {
  ...
  if (MOCK_MODE) {
    ...
    // import { BackendAdsProvider } from "@/integrations/providers/backend/backend-ads.provider";
