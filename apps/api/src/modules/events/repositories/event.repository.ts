...
export class EventRepository {
  ...
  create(tenantId: string, data: Partial<EventEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<EventEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<EventEntity>) {
