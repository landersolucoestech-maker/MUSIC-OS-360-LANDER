...
export class BillingRepository {
  ...
  create(tenantId: string, data: Partial<BillingEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<BillingEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<BillingEntity>) {
