...
export class ImportEngineService {
  ...
  /**
   * Template de importação: XLSX só com o cabeçalho (colunas importáveis do
   * contrato, mesmos rótulos pt-BR do export) — zero linhas de dado. Mesmo
   * guard de disponibilidade do validateFile/commit (nunca 500 por entidade
   * sem tabela física ou sem contrato de importação).
   */
  async buildTemplate(entity: string, tenantId: string | undefined): Promise<ImportTemplateResult> {
    ...
    const body = this.exportFormat.toXlsx(entity, def.importableColumns, []);
    return {
