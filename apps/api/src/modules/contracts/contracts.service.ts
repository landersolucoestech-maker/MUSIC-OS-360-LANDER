  ...
  async create(tenantId: string, userId: string, dto: CreateContractDto): Promise<Contract> {
    const [created] = await this.db
      ...
      .values({
        ...
        valor:       dto.value    ?? null,
        exclusivo:   false,
        data_inicio: dto.startsAt  ? new Date(dto.startsAt)  : null,
        data_fim:    dto.expiresAt ? new Date(dto.expiresAt) : null,
        arquivo_url: dto.fileUrl  ?? null,
        observacoes: null,
  ...
  async update(tenantId: string, userId: string, id: string, dto: UpdateContractDto): Promise<Contract> {
    ...
    const [updated] = await this.db
      ...
      .set({
        ...
        ...(dto.value     != null && { valor:       dto.value }),
        ...(dto.startsAt  != null && { data_inicio: new Date(dto.startsAt) }),
        ...(dto.expiresAt != null && { data_fim:    new Date(dto.expiresAt) }),
        ...(dto.fileUrl   != null && { arquivo_url: dto.fileUrl }),
