...
export class NotificationRepository {
  ...
  create(tenantId: string, data: Partial<NotificationEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<NotificationEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<NotificationEntity>) {
