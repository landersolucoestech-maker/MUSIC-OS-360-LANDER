...
export class CampaignRepository {
  ...
  create(tenantId: string, data: Partial<CampaignEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<CampaignEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<CampaignEntity>) {
