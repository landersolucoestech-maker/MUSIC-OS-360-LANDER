...
export class BriefingRepository {
  ...
  create(tenantId: string, data: Partial<BriefingEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<BriefingEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<BriefingEntity>) {
