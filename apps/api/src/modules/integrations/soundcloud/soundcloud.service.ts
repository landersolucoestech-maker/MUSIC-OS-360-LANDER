...
export class SoundCloudService extends IntegrationBaseService {
  ...
  isConfigured(): boolean {
    return !!(this.config.get('SOUNDCLOUD_CLIENT_ID'));
  }

  private get clientId(): string {
    return this.config.get<string>('SOUNDCLOUD_CLIENT_ID') ?? '';
  }
  ...
  async getProviderStatus(tenantId: string) {
    return this.getStatus(tenantId, this.PROVIDER);
  }

  async disconnectProvider(tenantId: string): Promise<void> {
    await this.disconnect(tenantId, this.PROVIDER);
  }
  ...
  async resolveUser(url: string) {
    ...
    return {
      ...
      following:   d.followings_count ?? 0,
      tracksCount: d.track_count ?? 0,
      avatar:      d.avatar_url ?? '',
  ...
  async searchTracks(query: string, limit = 10) {
    ...
    const res = await fetch(
      `${SC_API}/tracks?q=${encodeURIComponent(query)}&limit=${limit}&client_id=${cid}`,
    );
    if (!res.ok) return [];
