...
export class LeadsController {
  ...
  create(@CurrentTenant() t: { id: string }, @CurrentUser() _u: any, @Body() dto: CreateLeadDto) { return this.svc.create(t.id, dto); }

  @Patch(':id') @RequireRole('editor') @Audit('lead.updated') @ApiOperation({ summary: 'Actualizar lead' })
  update(@CurrentTenant() t: { id: string }, @CurrentUser() _u: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateLeadDto) { return this.svc.update(t.id, id, dto); }
