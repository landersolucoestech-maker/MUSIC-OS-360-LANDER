...
export class LeadsController {
  ...
  list(@CurrentTenant() t: { id: string }, @Query() q: QueryLeadDto) { return this.svc.list(t.id, q); }
  ...
  create(@CurrentTenant() t: { id: string }, @CurrentUser() u: any, @Body() dto: CreateLeadDto) { return this.svc.create(t.id, u?.sub ?? '', dto); }

  @Patch(':id') @RequireRole('editor') @Audit('lead.updated') @ApiOperation({ summary: 'Actualizar lead' })
  update(@CurrentTenant() t: { id: string }, @CurrentUser() u: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateLeadDto) { return this.svc.update(t.id, u?.sub ?? '', id, dto); }
