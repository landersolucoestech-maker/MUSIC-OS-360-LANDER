...
export class ProjectsController {
  ...
  create(@CurrentTenant() t: { id: string }, @CurrentUser() _u: any, @Body() dto: CreateProjectDto) { return this.svc.create(t.id, dto); }

  @Patch(':id') @RequireRole('editor') @Audit('project.updated') @ApiOperation({ summary: 'Actualizar projecto' })
  update(@CurrentTenant() t: { id: string }, @CurrentUser() _u: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateProjectDto) { return this.svc.update(t.id, id, dto); }
