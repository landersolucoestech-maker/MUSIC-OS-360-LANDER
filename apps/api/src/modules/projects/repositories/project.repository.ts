...
export class ProjectRepository {
  ...
  create(tenantId: string, data: Partial<ProjectEntity>) {
    const entity = this.repo.create({ ...data, tenant_id: tenantId } as Partial<ProjectEntity> & { tenant_id: string });
    return this.repo.save(entity);
  }

  update(tenantId: string, id: string, data: Partial<ProjectEntity>) {
