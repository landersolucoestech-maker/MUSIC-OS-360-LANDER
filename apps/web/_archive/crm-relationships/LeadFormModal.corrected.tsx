...
function SectionHeader({ title }: { title: string }) {
  return (
    <p className="text-sm font-semibold uppercase tracking-wider text-muted-foreground border-b pb-1 pt-2">
      {title}
    </p>
  );
}
...
export function LeadFormModal({
  ...
  // ✅ FIX 1: initialKey sem ternário — sempre reflete initialValue atual,
  // garantindo reset correto mesmo quando o modal reabre com o mesmo `open=true`
  // mas um initialValue diferente (ex: editar lead A → editar lead B sem fechar).
  ...
  // ✅ FIX 2: showInfluenciador não depende mais de !showEvento.
  // O bloco de Evento já cuida do caso influenciador + producao_eventos.
  // O bloco de Influenciador aparece para todos os outros serviços do tipo influenciador.
  const showInfluenciador = tipo === TIPO_LEAD_INFLUENCIADOR && !showEvento;

  // ✅ showEmpresario mantido — bloco JSX adicionado abaixo (FIX 4)
  const showEmpresario = tipo === TIPO_LEAD_EMPRESARIO;
  ...
  const showArtistaBandaAgencia    = tipo === "agencia"          && ["campanhas_artistas", "contratacao_artistas"].includes(servico);
  const showInfluenciadorAgencia   = tipo === "agencia"          && servico === "influenciadores";
  ...
  const showCampoArtista =
    ...
    showArtistaBandaAgencia    ||
    showInfluenciadorAgencia   ||
    showInfluenciadorTipoLead;

  const labelArtista =
    showInfluenciadorAgencia || showInfluenciadorTipoLead
      ? "Nome do Influenciador"
      : showArtistaBandaEmpresario || showArtistaBandaAgencia
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid gap-4 py-4">
              ...
              <SelectField
                ...
                onChange={(v) => {
                  ...
                  setValues((prev) => ({
                    ...
                    // ✅ FIX 3: limpa sub-objetos ao trocar tipo para evitar
                    // dados de contexto errado reaparecerem se o usuário voltar
                    evento:               { ...EVENTO_DEFAULT },
                    campanha:             { ...CAMPANHA_DEFAULT },
                    influenciador:        { ...INFLUENCIADOR_DEFAULT },
                    empresario:           { ...EMPRESARIO_DEFAULT },
                  }));
                }}
            ...
            <>
              ...
              <Field label="Necessidades Adicionais">
                <Textarea
                  ...
                  onChange={(e) => setInfluenciador("necessidades_adicionais", e.target.value)}
          ...
          {/* ══════════════════════════════════════
              DETALHES DO EMPRESÁRIO
              ✅ FIX 4: bloco que estava declarado mas nunca renderizado
          ══════════════════════════════════════ */}
          {showEmpresario && (
            <>
              <SectionHeader title="Detalhes do Empresário" />
