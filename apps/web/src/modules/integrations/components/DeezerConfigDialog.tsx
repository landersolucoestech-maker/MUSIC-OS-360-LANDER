 ...
 * Credenciais são persistidas em sessionStorage com prefixo musicos360_.
 */
...
import { CheckCircle, XCircle, Loader2, Trash2, ExternalLink, Music2 } from "lucide-react";
import {
  useDeezerStatus,
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-orange-500/10">
              <Music2 className="h-5 w-5 text-[#FF0092]" />
            </div>
            <div>
              <DialogTitle className="text-base">Deezer</DialogTitle>
              <DialogDescription className="text-xs">
                Conecte via App ID e Secret Key do Deezer Developers para importar relatórios de streams e royalties.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
            ...
            <div className="flex items-center justify-between rounded-lg border bg-muted/30 px-4 py-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                {isConnected && status?.app_id && (
                  <span className="text-muted-foreground">· App {status.app_id.slice(0, 12)}…</span>
                )}
              </div>
