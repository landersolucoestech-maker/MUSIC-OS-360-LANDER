...
interface AnexoFile { nome: string; dataUrl: string | null; }
...
const schema = z.object({
  ...
  client_types: z.array(z.enum(["artista", "pessoa_fisica", "pessoa_juridica"])).min(1, "Selecione ao menos um tipo"),
...
export function ServiceTypeFormModal({ open, onOpenChange, serviceType, onSave, existingSlugs }: ServiceTypeFormModalProps) {
  ...
  useEffect(() => {
    ...
    if (serviceType) {
      ...
      form.reset({ name: "", slug: "", description: "", category: "", client_types: ["artista"], financial_model: "valor_fixo", requires_royalties: false, requires_fixed_value: true, requires_advance: false, requires_financial_support: false, allow_installments: false, default_financial_category: "", active: true, sort_order: 1 });
      setParticipants([]);
      setClausulas([]);
      setMusicWork(DEFAULT_MUSIC_WORK);
      setSignatureSettings(DEFAULT_SIGNATURE);
      setBranding(DEFAULT_BRANDING);
      setFinancialCurrency("BRL");
      setFinancialFrequency("unico");
      setFinancialPenalty("");
              ...
              <div className="px-6 py-4 space-y-4">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        {form.formState.errors.client_types && <p className="text-xs text-destructive">{form.formState.errors.client_types.message}</p>}
                      </div>
                    </CardContent>
                  </Card>
                          ...
                          {participants.map((p, idx) => (
                            <div key={p.id} className="rounded-lg border border-border p-3 space-y-2">
                              ...
                              <div className="flex flex-wrap gap-1">
                                ...
                                {p.variables.length > 6 && <Badge variant="muted" className="text-[10px]">+{p.variables.length - 6} mais</Badge>}
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {allVariables.length > 20 && <Badge variant="muted" className="text-[10px]">+{allVariables.length - 20} mais</Badge>}
                      </div>
                    </CardContent>
                  </Card>
                        ...
                        <div className="space-y-4">
                          {clausulas.map((clausula, index) => (
                            <div key={clausula.id} className="rounded-lg border border-border p-4 space-y-3">
                              ...
                              <div className="space-y-2">
                                <Label>Título</Label>
                                <Input value={clausula.titulo} onChange={(e) => setClausulas((prev) => prev.map((c) => c.id === clausula.id ? { ...c, titulo: e.target.value } : c))} placeholder="Ex: DO OBJETO" />
                              </div>
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                          ...
                          <div className="space-y-2">
                            ...
                            <Select value={signatureSettings.provider} onValueChange={(v) => setSignatureSettings((s) => ({ ...s, provider: v as SignatureSettings["provider"] }))}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="docusign" disabled>DocuSign (em breve)</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                ...
                <TabsContent value="preview" className="mt-0">
                  <Card>
                    ...
                    <CardContent>
                      <div
                        className="rounded-lg border border-border bg-white dark:bg-zinc-950 text-foreground p-8 min-h-[500px]"
                      ...
                      >
                        ...
                        {clausulas.length === 0 ? (
                          ...
                          <div className="space-y-4">
                            {clausulas.map((c) => {
                              const varMap = new Map(allVariables.length > 0 ? allVariables.map((v) => [v.key, v.example]) : SYSTEM_VARIABLES.map((v) => [v.key, v.example]));
