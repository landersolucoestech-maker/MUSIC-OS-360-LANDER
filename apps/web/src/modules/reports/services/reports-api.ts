 ...
 *
 * FASE 2.5 — cliente da Central de Relatórios. Consome EXCLUSIVAMENTE os
 * endpoints oficiais do backend (fonte única da verdade). Sem mock, sem
 * registry/contrato/label no frontend.
 */
...
export const reportsApi = {
  ...
  /** GET /reports/entities/:entity/export — JSON estruturado. */
  exportJson: (entity: string, params: ExportParams) =>
    api.get<Record<string, unknown>>(`/reports/entities/${entity}/export?${buildExportQuery({ ...params, format: "json" })}`),

  /**
   * GET /reports/entities/:entity/export — CSV/XLSX como Blob (download).
...
/** Lê um arquivo para o body de importação (CSV/JSON = texto; XLSX = base64). */
